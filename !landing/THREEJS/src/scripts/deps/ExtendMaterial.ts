...
function extend<T extends THREE.Material>(source: T, object) {
	...
	if (object.override) {
		for (const name in object.override) {
	...
	if (base && base.templates && base.templates.length) {
		...
		for (const template of base.templates) patchShader(properties, template, mixUniforms, defines);

		// Linked uniforms: Assign linked uniforms of base template material

		for (const name in base.uniforms) {
	...
	for (const name in defines) if (defines[name] === false) delete defines[name];
