import { PostProcessRenderEffect, Scene, Engine, SceneLoader, Color4, Vector3, DefaultRenderingPipeline, Effect, PostProcess, Texture, type ILoadingScreen } from "@babylonjs/core"
...
export default class THREED {
    ...
    animate() {
        ...
        const offset = new Vector3(this.mouse_x * -0.00007, this.mouse_y * -0.00005, this.frames_count * this.rotation_speed)
    ...
    create_pipeline() {
        ...
        const distort_postprocess = new PostProcess("distort", "distort", [], [], 1, this.camera, Texture.TRILINEAR_SAMPLINGMODE, this.scene.getEngine(), false, null, 0)
        const distort_postprocess_effect = new PostProcessRenderEffect(this.scene.getEngine(), "distorteffect", () => { return [distort_postprocess] })
        ...
        if (default_pipeline.isSupported) {
            ...
            default_pipeline.bloomThreshold = .4;
            default_pipeline.bloomWeight = .8;
