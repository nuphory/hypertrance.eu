		...
		<section class="w-full h-screen">
			<div class="absolute right-[15vw] bottom-[20vh] bg-bg_island p-12">
				...
				<span class="font-sussnord text-xl flex justify-between mt-6 p-2">
					<button class="button-secondary p-4 m-2" on:click={()=> goto("app/store/product/hypertrance-sample-pack-01")}>buy now</button>
					<button class="button-neutral p-4 m-2"
						>learn more</button
					>
				</span>
			</div>
			<div
				class="w-full h-full bg-bg_base block fixed -z-10 [background:url('/assets/img/first_frame.webp')_no-repeat_fixed_center]"
			>
